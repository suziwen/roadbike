...
const astCacheKey = node =>
  `transformer-remarkx-markdown-ast-${
    node.internal.contentDigest
  }-${pluginsCacheStr}-${pathPrefixCacheStr}`
const htmlCacheKey = node =>
  `transformer-remarkx-markdown-html-${
    node.internal.contentDigest
  }-${pluginsCacheStr}-${pathPrefixCacheStr}`
const htmlAstCacheKey = node =>
  `transformer-remarkx-markdown-html-ast-${
    node.internal.contentDigest
  }-${pluginsCacheStr}-${pathPrefixCacheStr}`
const headingsCacheKey = node =>
  `transformer-remarkx-markdown-headings-${
    node.internal.contentDigest
  }-${pluginsCacheStr}-${pathPrefixCacheStr}`
const tableOfContentsCacheKey = node =>
  `transformer-remarkx-markdown-toc-${
    node.internal.contentDigest
  }-${pluginsCacheStr}-${pathPrefixCacheStr}`
...
module.exports = (
  ...
  return new Promise((resolve, reject) => {
    ...
    const HeadingLevelsx = new GraphQLEnumType({
      name: `HeadingLevelsx`,
      values: {
        h1: { value: 1 },
        h2: { value: 2 },
    ...
    const ExcerptFormatsx = new GraphQLEnumType({
      name: `ExcerptFormatsx`,
      values: {
        PLAIN: { value: `plain` },
        HTML: { value: `html` },
      },
